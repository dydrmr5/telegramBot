...
# bot commands = /start
@bot.message_handler(commands=["%s" %td.bot_commands[0]])
def get_user_info(message):
	...
	bot.send_message(
		message.chat.id,
		"%s, %s, %s.\n%s" % (td.bot_greetings(), user, td.waving_hand(), td.bot_start())
	)


# bot commands = /hi
@bot.message_handler(commands=["%s" % td.bot_commands[1]])
def sent_greets(message):
	user = message.from_user.first_name
	bot.reply_to(message, "%s %s %s" % (td.bot_greetings(), user, td.waving_hand())
	)


# bot commands = /menu
@bot.message_handler(commands=["%s" % td.bot_commands[2]])
def sent_info(message):
	bot.reply_to(message, td.bot_show_menu())


# bot commands = /jenis
@bot.message_handler(commands=["%s" % td.bot_commands[3]]) 
...
@bot.message_handler(commands=["%s" % td.bot_commands[4]])
def show_pengertian(message):
	bot.reply_to(message, td.show_pengertian_peribahasa())


# bot commands = /lapor
@bot.message_handler(commands=["%s" % td.bot_commands[5]])
def report_to_dev(message):
	...
	bot.reply_to(
		message,
		"Bila ada masalah, kritik ataupun saran,\nsampaikan ke developer ya kak %s." % td.happy_emoji(),
		reply_markup=button
	)


# bot commands = /peribahasa_bidal
@bot.message_handler(commands=["%s" % td.bot_commands[6]])
...
@bot.message_handler(commands=["%s" % td.bot_commands[7]])
...
@bot.message_handler(commands=["%s" % td.bot_commands[8]])
...
@bot.message_handler(commands=["%s" % td.bot_commands[9]])
...
@bot.message_handler(commands=["%s" % td.bot_commands[10]])
...
def custom_messages(message):
	...
	if "Bidal" in message.text:
		bot.reply_to(message, "%s" % td.get_bidal())
	elif "Pepatah" in message.text:
		bot.reply_to(message, "%s" % td.get_pepatah())
	elif "Perumpamaan" in message.text:
		bot.reply_to(message, "%s" % td.get_perumpamaan())
	elif "Tamsil" in message.text:
		bot.reply_to(message, "%s" % td.get_tamsil())
	elif "Semboyan" in message.text:
		bot.reply_to(message, "%s" % td.get_semboyan())
	elif "Menu" in message.text:
		bot.reply_to(message, "%s" % td.bot_show_menu())
